...
sed -i "s/server 10.8.0.0 255.255.255.0/server ${SERVER_POOL} nopool\nifconfig-pool ${DHCP_POOL}/g" /etc/openvpn/server/server.conf
...
echo "ifconfig-push ${PROXY1_IP}" > /etc/openvpn/server/ccd/proxy1
echo "ifconfig-push ${PROXY2_IP}" > /etc/openvpn/server/ccd/proxy2
