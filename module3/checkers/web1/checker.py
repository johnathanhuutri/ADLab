    ...
    def put(self, flag_id: str, flag: str, vuln: str):
        if int(vuln)==1:
            try:
                ...
                import traceback
                self.cquit(Status.MUMBLE, 'put failed', traceback.print_exception(e))
        else:
            try:
                client = paramiko.SSHClient()
                client.set_missing_host_key_policy(paramiko.AutoAddPolicy())
    ...
    def get(self, flag_id: str, flag: str, vuln: str):
        if int(vuln)==1:
            try:
                ...
                client.close()
                res = stdout.readline().decode()
                if res!=flag:
                    self.cquit(Status.CORRUPT, 'get failed', f'Flag: {res}')
            except Exception as e:
                import traceback
                self.cquit(Status.CORRUPT, 'get failed', traceback.print_exception(e))
        else:
            try:
                client = paramiko.SSHClient()
                client.set_missing_host_key_policy(paramiko.AutoAddPolicy())
